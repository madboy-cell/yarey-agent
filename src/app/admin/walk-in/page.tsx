...
function WalkInDashboardContent() {
    ...
    const handleValidateVoucher = (code: string): Voucher | null => {
        ...
        // 2. Check for bound vouchers (partner/media promo codes) — these are unlimited
        const boundVoucher = vouchers.find(v => v.code === cleanCode && v.boundType && v.status === "ISSUED")
        if (boundVoucher) {
            return boundVoucher // Return as-is — cart will read boundType/discountPercent/boundEntityId
        }

        // 3. Check regular vouchers
        const voucher = vouchers.find(v => v.code === cleanCode && v.status === "ISSUED" && !v.boundType)
        if (voucher) {
            // Check Expiration
    ...
    const handleCheckout = async (paymentMethod: string) => {
        ...
        for (const guest of cart) {
            ...
            let revenue = 0
            const boundDiscountAmt = guest.boundDiscountPercent && guest.treatment
                ? Math.round(guest.treatment.price_thb * guest.boundDiscountPercent / 100) : 0
            if (guest.voucherId) {
                ...
                revenue = Math.max(0, rawPrice - discount - boundDiscountAmt)
            }
            ...
            // Calculate partner commission if booking via partner
            let partnerCommission = 0
            if (guest.boundPartnerId) {
                // Look up partner commission from circle_partners would require fetching — use a simple approach
                // Commission is calculated later in the Partners sub-tab from booking data
                // For now, store 0 and let the dashboard compute
                partnerCommission = 0
            }
            ...
            const newBooking: Omit<Booking, "id"> = { // ID generated by Firestore
                ...
                notes: `POS Booking. ${guest.manualDiscount ? `DISCOUNTED [-${guest.manualDiscount}]` : ""} ${guest.voucherCode ? `PREPAID [${guest.voucherCode}]` : ""} ${guest.boundVoucherCode ? `PROMO [${guest.boundVoucherCode} -${guest.boundDiscountPercent}%]` : ""} ${guest.sendConfirmation ? `Confirmation sent to ${guest.email}` : ""}. Nat: ${guest.nationality || 'Unknown'}`,
                isWalkIn: true,
                groupId: groupId,
                ...
                partnerId: guest.boundPartnerId || null,
                mediaId: guest.boundMediaId || null,
                boundVoucherCode: guest.boundVoucherCode || null,
                discountApplied: boundDiscountAmt,
                partnerCommission: partnerCommission,
            }
            batchPromises.push(bookingOps.add(newBooking))

            // 4. Increment bound voucher usage count
            if (guest.boundVoucherCode) {
                const bv = vouchers.find(v => v.code === guest.boundVoucherCode && v.boundType)
                if (bv) {
                    batchPromises.push(voucherOps.update(bv.id, { usageCount: (bv.usageCount || 0) + 1 }))
                }
            }
        }
        ...
        try {
            ...
            // Recalculate vibe stats after adding bookings
