...
export default function WalkInDashboard() {
    ...
    const handleCheckout = async () => {
        ...
        for (const guest of cart) {
            ...
            const newBooking: Omit<Booking, "id"> = { // ID generated by Firestore
                ...
                commissionAmount: selectedSalesmanId ? (revenue * (salesmen.find(s => s.id === selectedSalesmanId)?.commissionRate || 0)) : 0
            }
    ...
    return (
        <div className="h-screen w-screen overflow-hidden flex bg-background">
            ...
            {/* LEFT: Commander (65%) */}
            <div className="w-[65%] flex flex-col relative bg-gray-50/50">
                {/* Header */}
                <div className="p-6 pb-2 flex items-center justify-between gap-4">
                    <Link href="/admin" className="flex items-center text-foreground/40 hover:text-foreground transition-colors text-sm font-bold uppercase tracking-widest">
                        <ArrowLeft className="w-4 h-4 mr-2" /> Exit POS
                    </Link>

                    {/* Date Picker */}
                    <div className="flex items-center gap-2 bg-white px-4 py-2 rounded-lg border border-border/50 shadow-sm">
                        <Calendar className="w-4 h-4 text-foreground/40" />
                        <input
                            type="date"
                            value={selectedDate}
                            onChange={handleDateSelect}
                            className="bg-transparent text-sm font-bold uppercase tracking-wider text-foreground focus:outline-none cursor-pointer"
                        />
                    </div>

                    <div className="text-right">
                        <h1 className="font-serif text-2xl text-foreground">Sanctuary POS</h1>
                    </div>
                </div>
